class Batch::ThreedayslaterUpdate
  require 'selenium-webdriver'

  # 3日後のデータ取得
  def self.threedayslater_update
    puts 'start'
    puts '3daylater_data'
    puts DateTime.now
    ...
    search_date = (Date.today + 6).strftime('%Y-%m-%d')
    ...
    (0..((sum / 10) + 1)).each do |page|
      ...
      @urls.each do |url|
        ...
        # start_datetime
        begin
          start_date = driver.find_element(:xpath,
                                           "//p[contains(text(), '放送日時・内容')]/following-sibling::div[1]/time[1]").attribute('datetime').slice(0..9)
        rescue Selenium::WebDriver::Error::NoSuchElementError
          start_date = ''
        end

        begin
          start_time = driver.find_element(:xpath,
                                           "//p[contains(text(), '放送日時・内容')]/following-sibling::div[1]//span[3]").text
        rescue Selenium::WebDriver::Error::NoSuchElementError
          start_time = ''
        end
        ...
        # end_datetime
        begin
          end_date = driver.find_element(:xpath,
                                         "//p[contains(text(), '放送日時・内容')]/following-sibling::div[1]/time[2]").attribute('datetime').slice(0..9)
        rescue Selenium::WebDriver::Error::NoSuchElementError
          end_date = ''
        end
